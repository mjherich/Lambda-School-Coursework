...
def sync_graph(g):
    ...
    r = blaine.requests.get(API_URL+"/api/rooms/adlist")
    ad_list = r.json()
...
def pick_up_items(arr):
    for item in arr[0:-1]:
        ...
        if item != '':
            #pick up Item
            post_data = {"name": item}
            headers= {"Authorization": f"Token {BLAINE_TOKEN}" } #we need to put the token here
...
# Navigate to Shop
path_to_shop = path_to_room_id(player, UNIQUE_ROOMS['pirate']['room_id'])
